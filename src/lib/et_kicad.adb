...
-- 				-- If the component contains more than one unit, write number of units.
			...
			-- If the unit_id is 0, the unit_cursor is not changed.
				...
				procedure locate_component ( 
					key			: in et_libraries.type_device_library_name.bounded_string;
			...
			begin -- read_field
				...
				case to_text_meaning (line => line, schematic => false) is
					...
					when reference =>
						...
						if strip_quotes (et_string_processing.field (line,2)) = type_component_prefix.to_string (tmp_prefix) then
							null; -- fine
						else
					...
					when others =>
						...
						if not component_entered then 
							...
							if et_string_processing.field (line,1) = et_kicad.def then
								...
								tmp_prefix := type_component_prefix.to_bounded_string (et_string_processing.field (line,3)); -- U
							...
							if exists (compose (
								...
								then
									...
									type_libraries.insert (
										container	=> tmp_component_libraries,
										key 		=> et_libraries.type_device_library_name.to_bounded_string (compose (
	...
	-- 				end if;
			...
			-- IMPORTANT AND CS: This procedure currently works under linux only !
				...
				-- The file is read into container "lines" which is then parsed to obtain the table content.
					...
					lib_uri		: et_libraries.type_device_library_name.bounded_string; -- CS not exact. see specs of type type_lib_table_entry
					-- CS lib_options
					-- CS lib_description
								...
								case section.name is
									...
									when SEC_URI =>
										case section.arg_counter is
											when 0 => null;
											when 1 =>
												lib_uri := to_device_library_name (to_string (arg));
											when others => too_many_arguments;
										end case;
...
-- 							 level => 1);
			...
			begin -- make_drawing_frame
				...
				if et_string_processing.field (et_kicad.line,1) = schematic_keyword_sheet then
					...
					sheet_count_total := to_sheet_number (et_string_processing.field (et_kicad.line,3));
...
-- 				log (to_string (et_kicad.line), log_threshold + 1);
				...
				if et_string_processing.field (et_kicad.line,1) = schematic_keyword_sheet_file then
					-- CS test field count					
					sheet_name.file := et_coordinates.to_schematic_file_name (et_string_processing.field (et_kicad.line,2));
		...
		case et_import.cad_format is
			when et_import.KICAD_V4 | et_import.KICAD_V5 =>
				...
				if not module_inserted then -- CS should never happen
					log_indentation_reset;
					log (message_error & "module " & et_coordinates.to_string (module_name) & " already in rig !");
					raise constraint_error;
				end if;
			...
			while junction_cursor /= type_junctions.no_element loop
				...
				if not segment_here then
					log (message_warning & "orphaned net junction at " 
						 & et_coordinates.to_string (element (junction_cursor).coordinates,
							...
							if same_path_and_sheet (
								...
								if (not distance.out_of_range) and distance.distance = et_coordinates.zero_distance then
									log (message_warning & "no-connection-flag misplaced on a net at " 
										& et_coordinates.to_string (element (no_connection_flag_cursor).coordinates, et_coordinates.module));
								end if;
							end if;
			...
			begin -- query_portlists
				...
				if flag_orphaned then
					...
					log (message_warning & "orphaned no_connection_flag found at " 
						 & et_coordinates.to_string (element (no_connection_flag_cursor).coordinates,
							et_coordinates.module));
				end if;
		...
		while module_cursor /= type_rig.no_element loop
			...
			bom_file_name := et_schematic.type_bom_file_name.to_bounded_string 
				...
				compose (
					containing_directory => compose 
						(
						containing_directory => compose (work_directory, et_coordinates.to_string (key (module_cursor))),
						name => et_export.directory_cam
						),
					name		=> et_coordinates.to_string (key (module_cursor)),

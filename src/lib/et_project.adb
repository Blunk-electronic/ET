...
-- 											end;
							...
							when SEC_COPPER =>
								...
								begin
									...
									if kw = keyword_center then -- center x 22.3 y 23.3
										...
										expect_field_count (line, 2);													
										board_track_circle.filled := et_pcb.to_filled (f (line, 2));

									elsif kw = keyword_fill_style then -- fill_style solid/hatched/cutout
										expect_field_count (line, 2);													
										board_track_circle.fill_style := et_pcb.to_fill_style (f (line, 2));
	...
	begin -- read_module_file
		...
		if template then
			...
			type_modules.insert (
				container	=> modules,
				key			=> to_module_name (compose (
					containing_directory 	=> containing_directory (file_name),
	...
	-- MODULES
		...
		procedure read_module_file_2 (module_file_handle : in directory_entry_type) is 
			file_name : string := simple_name (module_file_handle); -- motor_driver.mod
		begin
			read_module_file (file_name => file_name, template => false, log_threshold => log_threshold + 1);
		end;
		
		procedure read_module_file_old (module_file_handle : in directory_entry_type) is 
			file_handle : ada.text_io.file_type;
			file_name : string := simple_name (module_file_handle); -- motor_driver.mod
			module_cursor : type_modules.cursor;
			module_inserted : boolean;
	...
	begin -- open_project
		...
		if exists (to_string (project_name)) then
			...
			if more_entries (module_file_search) then
				search (current_directory, module_file_name_extension_asterisk, module_file_filter, read_module_file_2'access);
			else

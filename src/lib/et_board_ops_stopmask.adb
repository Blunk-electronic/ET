	...
	is
		...
		is
			...
			begin
				if not top.is_empty then
					lc := top.first;
					while lc /= pac_stop_arcs.no_element loop
...
-- CIRCLES:
	...
	is
		...
		begin
			case face is
				when TOP =>
					module.board.stopmask.top.placeholders.append (placeholder);

				when BOTTOM =>
					module.board.stopmask.bottom.placeholders.append (placeholder);
			end case;
		end query_module;
...
-- OBJECTS:
	...
	begin
		...
		if result_arc.cursor /= pac_stop_arcs.no_element then

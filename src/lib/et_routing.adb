...
package body et_routing is
	...
	is
		...
		procedure query_circle (c : in pac_polygon_circles.cursor) is
			...
			candidate_circle : constant type_polygon_circle := element (c);

			-- Compute the boundaries of the candidate circle.
			-- NOTE 2: The candidate circle is an edge of the polygon and thus has no width.
			-- NOTE 1: The candidate circle is an outer edge of the polygon ! It is not a cutout zone !
			boundaries : constant type_boundaries := get_boundaries (
			...
			procedure test_intersection is
				i_h : constant type_intersection_of_line_and_circle := get_intersection (pl_h, candidate_circle);
				i_c : constant type_intersection_of_line_and_circle := get_intersection (pl_c, candidate_circle);
				i_l : constant type_intersection_of_line_and_circle := get_intersection (pl_l, candidate_circle);

				-- If we have two intersections, then the one that is on the left, will 
				-- be the GO mark. Because here the circle causes the fill line to start.
				-- The intersection on the right will be the STOP mark, because here a fill
				-- line has to stop:
				procedure order_intersections (
					i : in type_intersection_of_line_and_circle;
					edge : in type_edge)
		...
		begin -- query_circle
			log (text => to_string (candidate_circle) & " " & to_string (boundaries),
				 level => log_threshold + 2);
	...
	begin -- get_polygon_proximity_points
		...
		log (text => "probing polygon circles ...", level => log_threshold + 2);
		iterate (segments.circles, query_circle'access);

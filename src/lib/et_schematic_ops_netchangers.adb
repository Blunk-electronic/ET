...
package body et_schematic_ops_netchangers is
	...
	is
		...
		begin
			...
			set_position (netchanger, to_netchanger_position (position));
	...
	begin
		log (text => "module " & to_string (module_cursor)
			 & " add netchanger at " & to_string (position)
	...
	begin
		log (text => "module " & to_string (module_cursor)
			& " set selected netchangers as moving",
			level => log_threshold);
		...
		log_indentation_down;
	end set_selected_netchangers_as_moving;

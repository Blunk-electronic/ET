...
-- 			unit_cursor := type_units.first (component.units);
-- 
-- 			-- In case the component has no units, abort.
-- 			if unit_cursor = type_units.no_element then
-- 				log_indentation_reset;
-- 				log (message_error & "generic component " 
-- 						& to_string (generic_name => type_components_library.key (component_cursor)) 
-- 						& " has no units !",
-- 					console => true);
-- 				raise constraint_error;
-- 			end if;
...
-- 						& to_string (unit_name => type_units_library.key (unit_cursor)) 
-- 						& " has no ports !");
-- 					--console => true);
-- 				--CS raise constraint_error;
-- 			end if;
